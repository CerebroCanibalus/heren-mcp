...
| **15 Centralized Tools** | Scenes, nodes, resources, scripts, signals, animations, shaders, validation and debug |
...
| **Integrated Validation** | Validates scenes, scripts, nodes and resources before applying changes |
...
| **No Godot Plugin** | Yes | No | No | **Yes** |
| **No Node.js** | No | No | No | **Yes** |
| **Batch operations** | No | Yes | No | **Yes** |
...
| **Memory** | 0MB | ~450MB | 0MB | **~75MB** |

### Feature Comparison

| Feature | Coding-Solo | GoPeak | tugcantopaloglu | **Heren** |
|---|---|---|---|---|
| **Persistent Daemon** | No | Yes | No | **Yes** |
| **No Plugin Required** | Yes | No | No | **Yes** |
| **Batch Operations** | No | Yes | No | **Yes** |
| **Signal Connection** | No | Yes | Yes | **Yes** |
| **Autoload Management** | No | Yes | Yes | **Yes** |
| **Debug Breakpoints** | No | Yes | No | **Yes** |
| **GPU Screenshots** | No | Yes | No | **Yes** |
| **Scene Validation** | No | No | No | **Yes** |
| **Shaders & Materials** | No | Yes | Yes | **Yes** |
| **TileMap/Terrain** | No | Yes | Yes | **Yes** |
| **Skeleton/Rigging** | No | No | No | **Yes** |
| **Spanish Docs** | No | No | No | **Yes** |
| **Installation** | `npx` (npm) | `npx` (npm) | npm | **`pip` (Python)** |

**What we have and they do not:** Persistent daemon with 0s setup, batch operations, automatic fallback, integrated validation, debug breakpoints, GPU screenshots, Spanish docs, no plugin or Node.js required.

**What they have and we do not:** LSP (GDScript autocomplete), DAP (advanced debugger), runtime inspection, input injection, asset library.
...
- **No language barrier**: because making games should not require speaking English
...
batch_tool(session_id="abc123", operations=[
    {"action": "add", "params": {"scene_path": "res://Player.tscn", "parent_path": ".", "node_type": "Sprite2D", "node_name": "Body"}},
    {"action": "add", "params": {"scene_path": "res://Player.tscn", "parent_path": "Body", "node_type": "CollisionShape2D", "node_name": "Hitbox"}},
    {"action": "save", "params": {"scene_path": "res://Player.tscn"}}
])
```
...
| **tilemap** | inspect_set, inspect_map, set_cell, terrain, pattern | TileMaps and TileSets |
| **resource** | create, read, update, delete, list | Resource management |
| **project** | setting, autoload, remove_autoload, shader_global | Project configuration |
...
```
AI Agent -> Heren MCP Server -> WebSocket -> Godot Daemon (headless)
                  |
                  v
...
**Made with love for the Ibero-American Godot community**
