...
def animation(action: str, **kwargs) -> dict:
...
def _action_create_player(session_manager, scene_path: str, parent_path: str = ".", player_name: str = "AnimationPlayer", **kwargs) -> dict:
...
def _action_create_animation(session_manager, scene_path: str, player_path: str, anim_name: str, length: float = 1.0, loop: bool = False, **kwargs) -> dict:
    session = session_manager.get_active_session()
    if not session:
        return {"success": False, "error": "No hay sesión activa"}
...
def _action_add_track(session_manager, scene_path: str, player_path: str, anim_name: str, track_type: str, node_path: str, property: str, **kwargs) -> dict:
    session = session_manager.get_active_session()
    if not session:
        return {"success": False, "error": "No hay sesión activa"}
...
def _action_add_key(session_manager, scene_path: str, player_path: str, anim_name: str, track_idx: int, time: float, value, transition: float = 1.0, **kwargs) -> dict:
    session = session_manager.get_active_session()
    if not session:
        return {"success": False, "error": "No hay sesión activa"}
...
def _action_state_machine(session_manager, scene_path: str, player_path: str, states: list = None, transitions: list = None, **kwargs) -> dict:
    session = session_manager.get_active_session()
    if not session:
        return {"success": False, "error": "No hay sesión activa"}
